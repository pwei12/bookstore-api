...
describe.only("Books", () => {
  describe("GET /books SUCCESSFULLY", () => {
    const route = "/books";
    test("Gets data of all the books", done => {
       request(app)
        .get(route)
        .set("Accept", "application/json")
        .expect(200)
        .expect("Content-Type", /json/)
        .expect(booksInventory, done);
    });

    test("Get a book's data by title using query", done => {
      return request(app)
        .get(route)
        .query({ title: "GHI" })
        .set("Accept", "application/json")
        .expect(200)
        .expect("Content-Type", /json/)
        .expect([{ title: "GHI", price: 35, quantity: 48, author: "Cecilia" }], done);
    });

    test("Get a book's data by author using query", done => {
      return request(app)
        .get(route)
        .query({ author: "John" })
        .set("Accept", "application/json")
